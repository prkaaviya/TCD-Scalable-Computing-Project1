...
class ImageSequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        # If no more files left to use, reset files (shuffle for randomness)
        if len(self.files) == 0:
        ...
        for i in range(actual_batch_size):
            ...
            # print(f"i {i} random_image_label {random_image_label}")
            random_image_file = self.files[random_image_label]
            # print(f"i {i} random_image_file {random_image_file}")
        ...
        # print(f"length of list(self.files.keys()) {len(list(self.files.keys()))}")
        return X, y
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        try:
            model.fit(training_data,
                                ...
                                epochs=args.epochs,
                                verbose=1)
            model.save_weights(args.output_model_name+'.h5')
